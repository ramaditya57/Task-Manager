      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title='Select Users'>
        <div className='space-y-4 h-[60vh] overflow-y-auto'>
          {allUsers.map((user) => (
            <div
              key={user._id}
              className='flex items-center gap-4 p-3 border-b border-slate-700 bg-slate-800 rounded-md'
            >
              <img
              ...
              <input
                ...
                className='w-4 h-4 text-primary bg-slate-700 border-slate-600 rounded-sm outline-none'
              />
            </div>
          ))}
        </div>
